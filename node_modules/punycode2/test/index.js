...
var testData = {
	...
	ucs2: [
		...
		{
			description: 'Unmatched high surrogate, followed by a surrogate pair, followed by an' +
		...
		{
			description: 'Unmatched low surrogate, followed by a surrogate pair, followed by an' +
...
test('punycode.ucs2.decode', function () {
	...
	each(allSymbols, function (string, codePoint) {
		deepEqual(punycode.ucs2.decode(string), [codePoint], 'Decoding symbol with code point ' +
			codePoint);
	});
...
test('punycode.toUnicode', function () {
	...
	*/
	each(testData.strings, function (object) {
		var message = 'Domain names (or other strings) that don\'t start with ' +
...
test('punycode.toASCII', function () {
	...
	*/
	each(testData.strings, function (object) {
		equal(punycode.toASCII(object.encoded), object.encoded,
